...
fn update_file(path: &str, years : Vec<Year>, organization: &str) -> io::Result<()> {
    ...
    let mut fh = try!(OpenOptions::new().read(true).write(true).open(path));
    ...
    let mut first_line_buff = String::new();
    {
        ...
        try!(br.read_line(&mut first_line_buff));
    }
    ...
    if !replacing_existing_notice {
        ...
        try!(slide_file_contents(&fh, 0, new_first_line.len() as isize));
    }
    else {
        ...
        try!(slide_file_contents(&fh, old_first_line.len(), slide_amount));
    }

    // Rewind to the start and write our notice line.
    try!(fh.seek(io::SeekFrom::Start(0)));

    try!(fh.write_all(new_first_line.as_bytes()));
...
fn slide_file_contents(rust_handle : &File, offset: usize, amount : isize) -> io::Result<()> {
    ...
    let file_length_64 : u64 = try!(rust_handle.metadata()).len();
    ...
    if amount < 0 { // We have to shrink the file.
        ...
        let mut mapping = try!(Mapping::open(fd, file_length));
        ...
        let mut mapping = try!(Mapping::open(fd, new_length as usize));
        unsafe { // memmove, a la Rust
            ptr::copy(mapping.ptr().offset(offset as isize),
                      mapping.mut_ptr().offset(offset as isize + amount),
                      file_length - offset);
